#!/usr/bin/env python3
"""
실험 5: OpenClaude 전체 하니스 vs 추출 하니스 vs Raw
=====================================================

OpenClaude(Claude Code 포크)의 전체 런타임 하니스(~21K 토큰)와
추출 시스템 프롬프트(~500 토큰)의 효과 차이를 검증.

3-Column (기존 Col1/Col2 결과 재사용, Col3만 신규):
  Col1: 바닐라 Raw (기존 결과)
  Col2: 바닐라 + 추출 하니스 (기존 결과)
  Col3: 바닐라 + OpenClaude 전체 하니스 (NEW)
"""
...
OPENCLAUDE_CLI = str(Path(__file__).parent / "openclaude" / "dist" / "cli.mjs")
...
def call_openclaude(user_prompt: str) -> dict:
    """OpenClaude CLI로 바닐라 Qwen3.5-27B 호출 (전체 하니스 적용)"""
    env = os.environ.copy()
    env["CLAUDE_CODE_USE_OPENAI"] = "1"
    env["OPENAI_BASE_URL"] = "https://dashscope-intl.aliyuncs.com/compatible-mode/v1"
    env["OPENAI_API_KEY"] = "sk-2a2c7474fabd4a229f48cd1615d85507"
    env["OPENAI_MODEL"] = "qwen3.5-27b"
    env["PATH"] = os.path.expanduser("~/.bun/bin") + ":" + env.get("PATH", "")

    cmd = ["node", OPENCLAUDE_CLI, "-p", "--output-format", "json"]
    start = time.time()
    try:
        r = subprocess.run(cmd, input=user_prompt, capture_output=True, text=True, timeout=180, env=env)
        ...
        try:
            ...
            return {
                ...
                "model": f"openclaude/qwen3.5-27b",
                "cost_usd": out.get("total_cost_usd", 0),
            }
            ...
            return {
                ...
                "model": "openclaude/qwen3.5-27b", "cost_usd": 0,
            }
...
def main():
    print("=" * 70)
    print("  실험 5: OpenClaude 전체 하니스 효과 검증")
    print(f"  과제 {len(TASKS)}개 × OpenClaude(바닐라 Qwen3.5-27B)")
    print(f"  하니스: Claude Code 전체 런타임 (~21K 토큰)")
    ...
    for i, task in enumerate(TASKS):
        print(f"\n{'━' * 70}")
        print(f"  [{i+1}/{len(TASKS)}] [{task['category'].upper()}] {task['name']}")
        print(f"{'━' * 70}")

        gen = call_openclaude(task["prompt"])
        ...
        entry = {
            ...
            "task_prompt": task["prompt"],
            "condition": "openclaude_vanilla",
            "condition_name": "OpenClaude + 바닐라 Qwen3.5-27B",
            "system_prompt_used": "(openclaude full harness ~21K tokens)",
        ...
        print(f"    ✓ GEN: {gen['response_length']} chars, {gen['elapsed']}s, "
              f"in={gen.get('input_tokens',0)} out={gen.get('output_tokens',0)} tok")

        print(f"    ⏳ Evaluating...")
        ev = evaluate_with_claude(task, gen["response"], "OpenClaude + 바닐라 Qwen3.5-27B")
        entry["evaluation"] = ev
    ...
    outfile = OUTPUT_DIR / f"openclaude_experiment_{ts}.json"
    with open(outfile, "w", encoding="utf-8") as f:
        json.dump({
            "meta": {
                "experiment": "openclaude_full_harness",
                "timestamp": datetime.now().isoformat(),
                "openclaude_version": "0.1.8",
                "model": "qwen3.5-27b (alibaba cloud intl)",
                "harness": "openclaude full runtime (~21K tokens)",
            },
            "results": all_results,
    ...
    if valid:
        ...
        avg_in_tokens = round(sum(r.get("input_tokens", 0) for r in valid) / len(valid))
        avg_time = round(sum(r.get("elapsed", 0) for r in valid) / len(valid), 1)

        print(f"\n{'=' * 70}")
        print(f"  OpenClaude + 바닐라 Qwen3.5-27B (전체 하니스)")
        print(f"  AVG: {avg} | Chars: {avg_chars} | InTok: {avg_in_tokens} | OutTok: {avg_tokens} | Time: {avg_time}s")
        print(f"{'=' * 70}")
